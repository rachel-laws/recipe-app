...
function Ketogenic() {
  ...
  const getKetogenic = async () => {
    const checkStorage = localStorage.getItem('ketogenic');

    if (checkStorage) {
      setKetogenic(JSON.parse(checkStorage));
    } else {
      //
      const api = await fetch(
        `https://api.spoonacular.com/recipes/random?apiKey=${process.env.REACT_APP_API_KEY}&number=12&tags=ketogenic`
      );
