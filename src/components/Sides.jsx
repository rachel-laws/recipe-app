...
function SideDish() {
  ...
  const getSideDish = async () => {
    const checkStorage = localStorage.getItem('sideDish');

    if (checkStorage) {
      setSideDish(JSON.parse(checkStorage));
    } else {
      //
      const api = await fetch(
        `https://api.spoonacular.com/recipes/random?apiKey=${process.env.REACT_APP_API_KEY}&number=12&tags=side-dish`
      );
